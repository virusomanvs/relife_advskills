...
- **`perkID`**: `int` ID перка.
- **`coefToBlood`**: `float` Коэфицент на который будет умножен ванильный реген шока. То есть, если coefToRegen = 2, то реген шока будет быстрее в два раза при активированом перке.
- **`coefToWater`**: `float` Коэфицент на который будет умножен ванильный реген шока. То есть, если coefToRegen = 2, то реген шока будет быстрее в два раза при активированом перке.
```json
[
