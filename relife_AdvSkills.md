...
```
## Различные настроки
- **`enableCatEyePerk`**: `bool` Отключает полностью кошачьи глазки. Необходимо при визуальных багах в прицелах (обычно с модом sVisual и его ответвлениями). Решения пока нет, только отключить.
