...
- **`PointsDecreaseAfterDeath`**:  `float` Значение от 0.0 до 1.0 на сколько в процентах останется кол-во опыта данного типа при смерти. То есть, 0.25 останется 25% от опыта, 0.5 останется 50% от опыта и так далее.
...
```
## Различные настроки
- **`enableCatEyePerk`**: `bool` Отключает или включает кошачьи глазки. Необходимо при визуальных багах в прицелах (обычно с модом sVisual и его ответвлениями). Решения пока нет, только отключить.
- **`enableCatEyePerkInUndergroundTrigger`**: `bool`  Отключает или включает кошачьи глазки в ванильных зонах затемнения.
