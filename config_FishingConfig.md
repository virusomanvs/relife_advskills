...
```
## FishingZones - Здесь вы можете указать зоны на карте где будут ловиться определенный вид рыб, либо вообще не будет ничего ловиться.
...
- **`allowItem`**: `TIntArray` Список ID для рыбы или предметов, не перепутайте категории!
